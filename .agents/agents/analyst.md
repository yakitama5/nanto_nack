...
分析時は以下の設計資料と照合し、実装が仕様と乖離していないか確認すること。

- `docs/table-design.md`: DBテーブル定義（スキーマ・カラム名の整合確認）
- `docs/custom-language-design.md`: カスタム言語仕様（変換ロジックの正確性確認）
- `docs/quizzes/<category>.md`: クイズ仕様（ミッション・クリア条件との整合確認）

## 📋 出力ガイドライン
- 分析結果は定量的かつ具体的に提示してください。
- 規約違反が見つかった場合は、該当する `.agents/rules/*.md` ファイルを引用して説明してください。
