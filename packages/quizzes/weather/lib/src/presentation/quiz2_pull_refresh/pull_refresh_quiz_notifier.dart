...
import 'pull_refresh_quiz_state.dart';

final pullRefreshQuizProvider =
    AutoDisposeNotifierProvider<PullRefreshQuizNotifier, PullRefreshQuizState>(
  PullRefreshQuizNotifier.new,
);

class PullRefreshQuizNotifier
    extends AutoDisposeNotifier<PullRefreshQuizState> {
  static const _quizId = 'weather_quiz2';
  Timer? _timer;

  @override
  PullRefreshQuizState build() {
    ref.onDispose(() => _timer?.cancel());

    // weatherAppProviderをlistenし、isRefreshingがtrueになったらクリア判定。
    // refreshWeather()が呼ばれた直後（リフレッシュ開始時点）をクリア条件とする。
    ...
    return PullRefreshQuizState.initial();
  }
  ...
  void startQuiz() {
    ...
    state = PullRefreshQuizState.initial().copyWith(
  ...
  void retry() {
    ...
    state = PullRefreshQuizState.initial();
