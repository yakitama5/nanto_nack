...
import 'increment_quantity_quiz_state.dart';

final incrementQuantityQuizProvider = AutoDisposeNotifierProvider<
    IncrementQuantityQuizNotifier, IncrementQuantityQuizState>(
  IncrementQuantityQuizNotifier.new,
);

class IncrementQuantityQuizNotifier
    extends AutoDisposeNotifier<IncrementQuantityQuizState> {
  static const _quizId = 'delivery_quiz2';
  static const _clearQuantity = 2;
  Timer? _timer;

  @override
  IncrementQuantityQuizState build() {
    ref.onDispose(() => _timer?.cancel());

    // deliveryAppProvider(browsing) を listen し、
    // orderQuantity が 2 になった瞬間にクリア判定を行う。
    ref.listen(deliveryAppProvider(DeliveryViewState.browsing), (prev, next) {
      if (state.status != QuizStatus.playing) return;
      if (prev?.orderQuantity != _clearQuantity &&
          next.orderQuantity == _clearQuantity) {
        unawaited(_onClear());
      }
    });

    return IncrementQuantityQuizState.initial();
  }

  void startQuiz() {
    if (state.status != QuizStatus.idle) return;
    // clock.now() を使用（DateTime.now() は使用禁止）
    state = IncrementQuantityQuizState.initial().copyWith(
  ...
  void retry() {
    ...
    state = IncrementQuantityQuizState.initial();
