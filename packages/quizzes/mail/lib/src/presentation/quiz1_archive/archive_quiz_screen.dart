...
import 'package:mail/src/presentation/quiz1_archive/archive_quiz_notifier.dart';
import 'package:quiz_core/quiz_core.dart';

class ArchiveQuizScreen extends ConsumerStatefulWidget {
  const ArchiveQuizScreen({super.key, this.onCompleted});

  final VoidCallback? onCompleted;

  @override
  ConsumerState<ArchiveQuizScreen> createState() => _ArchiveQuizScreenState();
}

class _ArchiveQuizScreenState extends ConsumerState<ArchiveQuizScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(archiveQuizProvider.notifier).startQuiz();
    });
  }

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(archiveQuizProvider);
    final s = context.s;
    final sq = context.sq;
    final missionText = s.quiz1.missionText;
            ...
            ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return MailListItem(
                  ...
                  onDismissed: (id) =>
                      ref.read(archiveQuizProvider.notifier).archiveMail(id),
                );
              },
            ),
              ...
              Positioned.fill(
                child: QuizResultOverlay(
                  ...
                  onRetry: () {
                    ...
                    ref.read(archiveQuizProvider.notifier).retry();
                  },
                  onNext: state.status == QuizStatus.correct
                      ? widget.onCompleted
                      : null,
                  onBack: () => Navigator.of(context).pop(),
                  insight: const _ArchiveInsight(),
                ),
              ),
          ],
        ),
