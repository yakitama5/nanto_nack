  ...
  group('GetDashboardDataUseCase', () {
    ...
    group('連続プレイ日数（ストリーク）計算', () {
      ...
      test('今日だけプレイしていればストリーク 1', () async {
        ...
        final useCase = GetDashboardDataUseCase(
          FakeDashboardRepository(
            history: [_makeResult(lastPlayedAt: today)],
          ),
        );
        ...
        final useCase = GetDashboardDataUseCase(
          FakeDashboardRepository(
            history: [
              _makeResult(
                quizId: 'q1',
                lastPlayedAt: today.subtract(const Duration(days: 2)),
              ),
              _makeResult(
                quizId: 'q2',
                lastPlayedAt: today.subtract(const Duration(days: 1)),
              ),
              _makeResult(quizId: 'q3', lastPlayedAt: today),
            ],
          ),
        );
    ...
    group('アクティビティ履歴', () {
      ...
      test('プレイ履歴がある日の clearCount が正しく集計される', () async {
        ...
        final useCase = GetDashboardDataUseCase(
          FakeDashboardRepository(
            history: [
              _makeResult(quizId: 'q1', lastPlayedAt: today, score: 800),
              _makeResult(quizId: 'q2', lastPlayedAt: today, score: 600),
            ],
          ),
        );
